...
public class AjoutController {
        ...
        if(!saisierue.getText().equals("") && !saisiecodepostal.getText().equals("") && !saisieville.getText().equals("")){
            if(toutesLesSurfaces != null && toutesLesTypesPieces != null && tousLesNomsEquipements != null){
                ...
                if(okay){
                    ...
                    Alert lalerte = new Alert(Alert.AlertType.CONFIRMATION);
                    lalerte.setContentText("Le logement a bien été ajouté !");
                    lalerte.show();
    ...
    public void onSelectNbPieces() {
        ...
        for (int i = 1; i <= nombreDePieces; i++) {
            ...
            pieceContainer.getChildren().addAll(labelTypePiece, selectTypePiece, labelSurfacePiece, surfacePiece, labelNbEquipement, selectnbequipements);
            ...
            selectnbequipements.getSelectionModel().selectedItemProperty().addListener((options, oldValue, newValue) -> {
                ...
                pieceContainer.getChildren().addAll(labelTypePiece, selectTypePiece, labelSurfacePiece, surfacePiece,labelNbEquipement, selectnbequipements, equipementContainer);
